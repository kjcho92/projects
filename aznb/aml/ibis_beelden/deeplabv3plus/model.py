...
def Deeplabv3(
    ...
    def resize(x):
        return tf.image.resize(x, size_before[1:3], method="bilinear")

    b4 = Lambda(lambda x: resize(x))(b4)
    # simple 1x1
    ...
    if backbone == "xception":
        ...
        def resize_2(xx):
            return tf.image.resize(xx, skip1.shape[1:3], method="bilinear")

        x = Lambda(lambda xx: resize_2(xx))(x)
    ...
    def resize_3(xx):
        return tf.image.resize(xx, size_before3[1:3], method="bilinear")

    x = Lambda(lambda xx: resize_3(xx))(x)
