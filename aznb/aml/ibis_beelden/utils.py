...
_ITEMS_TO_DESCRIPTIONS = {
    ...
    "labels_class": (
        "A semantic segmentation label whose size matches image."
...
BRINK_INFORMATION = DatasetDescriptor(
    splits_to_sizes={"train": 2144, "val": 267, "test": 267},
    num_classes=9,
    ignore_label=255,
    label_mapping={
        0: "background",
...
def get_files(data_folder, train_test_eval="train"):
    file_pattern = "*_%s-*"
    return glob.glob(
        os.path.join(data_folder, file_pattern % train_test_eval), recursive=True
    )


def get_dataset_for_generator(
    data_folder, train_test_eval,
):
    filenames = get_files(data_folder, train_test_eval)
    dataset = tf.data.TFRecordDataset(filenames=[filenames])

    @tf.function
    def _parse_image_function(example_proto):
        x = tf.io.parse_single_example(example_proto, KEYS_TO_FEATURES)
        return [
            tf.io.decode_jpeg(
                contents=x["image/encoded"], channels=3, name="xception_input"
            ),
            # x["image/filename"],
            # x["image/height"],
            # x["image/width"],
            tf.io.decode_png(
                contents=x["image/segmentation/class/encoded"], channels=1, name="label"
            ),
        ]

        # {
        # 'image': tf.io.decode_jpeg(contents=x["image/encoded"], channels=3, name="image"),
        # 'image_name': x['image/filename'],
        # 'height': x['image/height'],
        # 'width': x['image/width'],
        # 'labels_class': tf.io.decode_png(
        #     contents=x['image/segmentation/class/encoded'],
        #     channels=1),
        # }
...
def get_dataset(
    ...
    @tf.function
    def _parse_records(example_proto):
        x = tf.io.parse_single_example(example_proto, KEYS_TO_FEATURES)
        # print(x)
        image = tf.io.decode_jpeg(
            contents=x["image/encoded"],
            channels=3,
        ...
        original_image, image, label = preprocess_image_and_label(
            image,
            label,
            crop_height=crop_size_h,
            crop_width=crop_size_w,
            min_resize_value=min_resize_value,
            max_resize_value=max_resize_value,
            resize_factor=None,  # resize_factor,
            min_scale_factor=min_scale_factor,
            max_scale_factor=max_scale_factor,
            scale_factor_step_size=scale_factor_step_size,
        )

        # might need this for datagenerator todo
        # sample = {
        #     "xception_input": image, #.reshape(1, crop_size_h, crop_size_w, 3),
        ...
        # }
        # if label is not None:
        #     sample["label"] = label, #.reshape(1, crop_size_h, crop_size_w, 1)

        # Map image values from [0, 255] to [-1, 1].
        image = (2.0 / 255.0) * tf.cast(image, tf.float32) - 1.0

        return (
            tf.reshape(image, (crop_size_h, crop_size_w, 3)),
            tf.reshape(
                label,
                # undid one-hot encoded to test SparseCategoricalCrossentropy, and that worked and takes less memory.
                # tf.one_hot(label, num_classes, on_value=1.0, off_value=0.0),
                (crop_size_h, crop_size_w, 1),
            ),
        )

    return raw_dataset.map(_parse_records)
...
def preprocess_image_and_label(
  ...
  """
    ...
    mean_pixel = tf.reshape(_MEAN_PIXEL, [1, 1, 3])
...
def randomly_scale_image_and_label(image, label=None, scale=1.0):
  ...
  """
    ...
    # 4D tensors as input.
    image = tf.squeeze(
        tf.image.resize(tf.expand_dims(image, 0), new_dim), [0]  # align_corners=True),
    )
    if label is not None:
        label = tf.squeeze(
            tf.image.resize(
